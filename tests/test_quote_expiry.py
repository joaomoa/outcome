...
def test_accept_rejected_when_quote_expires(db_session, participants):
    eng = RfqEngine(db_session, FIXED_AT)
    ...
    assert get_balance(db_session, participants["mm1"]).reserved == reserved

    late = RfqEngine(db_session, FIXED_AT + timedelta(seconds=120))
    with pytest.raises(QuoteExpiredError):
        late.accept(request_id)

    assert eng.get_request_status(request_id) == RequestStatus.FAILED
    mm = get_balance(db_session, participants["mm1"])
